...
typedef vector<UnaryWeightedClause> BinaryClauseList;
...
struct ResolutionInfo {
  ...
  ResolutionInfo( int f, int s, int w ) { first = f; second = s; weight = w; }
...
list<UnaryWeightedClause> *unit_resolution_list;
#endif
...
#ifndef RESOLUTION_RULE
inline void remove_clause_db( int penultimate, int last, int weight ) {
  BinaryClauseList::iterator i = find( clausesbc[ penultimate ].begin(), clausesbc[ penultimate ].end(), 
				       UnaryWeightedClause( last, weight ) );
...
#ifdef RESOLUTION_RULE
inline void remove_clause_db ( int penultimate, int last, int weight ) {
  //cout << "Remove clause " << penultimate << " " << last << " " << weight << endl;
  BinaryClauseList::iterator begin = clausesbc[ penultimate ].begin();
  while( weight > 0 ) {
#ifdef MODE_TEST
  if ( clausesbc[ penultimate ].empty() ) { 
    cout << "Var: " << penultimate << endl;
    critical_error( "Empty clausesbc" ); 
  }
#endif

  BinaryClauseList::iterator i = find( begin, clausesbc[ penultimate ].end(), last );
  
#ifdef MODE_TEST
    if( i == clausesbc[ penultimate ].end() ) { 
      cout << clausesbc[ penultimate ] << endl;
      cout << "Distance: " << clausesbc[ penultimate ].end() - i << endl;
      cout << "Looking for " << last << " " << weight << endl;
      critical_error( "Clause not found" ); 
    }
#endif
    if ( weight > i->second ) {
      weight -= i->second;
...
void back( int var, int index_clause ) {
  ...
  while( clauses[ index_clause ].apn_value_abs == var 
	 && index_clause < num_long_clauses ) {
    if ( previous_not_satisfied( clauses[ index_clause ] ) ) {
      //cout << "Back Index Clause " << index_clause << endl;
...
int forward_checking( int var_in, int index_clause, 
		      //int unit[ 2*MAX_VARS ], int weights[ 2*MAX_VARS ], 
		      int& length_cuc, int& length_star, int& unsat ) {
  int fc = 0, inc;
...
/*
void undo_forward_checking( int unit[ 2*MAX_VARS ], int weights[ 2*MAX_VARS ], int length  ) {
  for( int i = 0; i < length; i++ ) {
    unit_clauses[ unit[ i ] ] += weights[ i ];
  }
}
*/
...
#endif
    unsat += forward_checking( var, num_long_clauses, length_cuc, length_star, unsat ); //unit, weights, length_unit );
...
#ifdef LOOK_AHEAD
  fc = forward_checking( var, new_index_clause, length_cuc, length_star, unsat ); //unit, weights, length_unit );
#else
  fc = 0;
#endif
